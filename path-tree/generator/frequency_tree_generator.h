...
namespace frequency_tree_generator {
...
void single_freuqnecy_tree_generator(Node& root,std::vector<Supernode *>& tree_paths);
...
void frequency_trees_collection_generator(std::vector<Node>& trees_collection, std::vector<Supernode* >& Supernodes_collection);


void single_frequency_tree_generator(Node& root,std::vector<Supernode *>& tree_paths){
    
    std::vector<int> heavy_id;
    
    first_time_tag(root, heavy_id);
    // for(auto id:heavy_id)
...
void first_time_tag(Node& node,std::vector<int>& heavy_id){
    ...
    for(int i=0;i<(int)node.children_count();i++){
        int subtree_frequency=node.get_children()[i].get_tree_frequency();
        if(subtree_frequency>max_frequency){
            max_frequency=subtree_frequency;
            max_index=i;
        }
    }
    //std::cout<<"max index: "<<max_index<<" max frequency: "<<max_frequency<<std::endl;
...
void frequency_trees_collection_generator(std::vector<Node>& trees_collection, std::vector<Supernode* >& Supernodes_collection){
    ...
    for(int i=0;i<tree_number;i++){
        ...
        std::cout<<"generate freuqnecy-tree: "<<i<<std::endl;
        std::vector<Supernode *> tree_paths;

        single_frequency_tree_generator(trees_collection[i], tree_paths);
